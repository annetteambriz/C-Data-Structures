...
/**
 * Takes a TreeNodePtr and prints the following
 * relative to that TreeNodePtr:
 * The node's parent (if any)
 * The node's left child (if any)
 * The node's right child (if any)
 * The node's left sibling (if any)
 * The node's right sibling (if any)
 *
 * @param nodeP
 */
void visitAll(TreeNodePtr nodeP)
{
    // Using the TreeNodePtr pointer, access the TreeNode structs' left
    // right and parent.
    // Prints relatives' data as a string.
